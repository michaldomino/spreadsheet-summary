...
class SpreadsheetSummaryService:
    ...
    def calculate_sum(self, column_names: 'List[str]', start_row=0, end_rows_skipped=0):
        end_row = -end_rows_skipped if end_rows_skipped > 0 else None
        return self._data_frame[column_names][start_row:end_row].sum()

    def calculate_average(self, column_names: 'List[str]', start_row=0, end_rows_skipped=0):
        end_row = -end_rows_skipped if end_rows_skipped > 0 else None
